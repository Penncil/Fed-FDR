...
######scale plot
result1<-readRDS("n500p500corr0.5signal7supp.rds")
...
samp<-rep(c("Sparsity level"),nrow(res1))
res1<-data.frame(res1,samp)
colnames(res1)<-c("method","type","Correlation","value","scale")

result1<-readRDS("p500corr0.5signal7samplesize.rds")
result1[which(is.na(result1))]<-0
n<-dim(result1)[1]
value<-c(result1[,1:2])
type<-rep(c("FDR","Power"),each=n)
...
samp<-rep(c("Local sample size"),nrow(res2))
res2<-data.frame(res2,samp)
colnames(res2)<-c("method","type","Correlation","value","scale")

result1<-readRDS("n500p500corr0.3signal7K.rds")
result1[which(is.na(result1))]<-0
n<-dim(result1)[1]
value<-c(result1[,1:2])
type<-rep(c("FDR","Power"),each=n)
...
########lambda tuning plot
result1<-readRDS("n500p500corr0.5signal7lambda.rds")
result1[which(is.na(result1))]<-0
n<-dim(result1)[1]
value<-c(result1[,1:2])
type<-rep(c("FDR","Power"),each=n)
labelname<-c("Fed-FDR","Fed-FDR-SCAD")
method<-rep(labelname,n/length(labelname))
Correlation<-rep(result1[,3],2)
a<-data.frame(value,type,method,Correlation)
res3<-aggregate(a$value,by=list(a$method,a$type,a$Correlation),mean )
samp<-rep(c("Scale of tuning parameter"),nrow(res3))
res3<-data.frame(res3,samp)
colnames(res3)<-c("method","type","Correlation","value","scale")


#cols <- c("#FFB000","#8C3333","#D2691E","#FF7256","#016A70","#7A9D54","#000000","#F2EE9D")
cols <- c("#8C3333","#D2691E")
plot <- ggplot2::ggplot(res3, ggplot2::aes(x = Correlation, y = value, color = method))
plot2 <- plot + 
  geom_point(aes(shape = method,size=method))+
